...
void handleKeypad()
{
    ...
    if (!key)
    {
        if (currentState != SYS_IDLE && (millis() - lastActivityTime > TIMEOUT_TIME))
            showIdle();
        return;
    }
    wakeUpLCD();
...
void loop()
{
    ...
    if (digitalRead(BUTTON_PIN) == LOW)
    {
        delay(50);
        if (digitalRead(BUTTON_PIN) == LOW)
            unlock("button", "Nut Bam");
    }
    maintainConnections();
    if (mqttClient.connected())
        mqttClient.loop();
    if (isLCDBacklightOn && (millis() - lastLCDActivity > LCD_TIMEOUT))
    {
        lcd.noBacklight();
